...
def chatgpt_answer_question(question_name, answer_str, chatgpt_index=0, knowledge_points='', tips=''):
    ...
    def requests_post_0(question):
        try:
            headers = {'Referer': 'https://chat18.aichatos.xyz/',
                       'Origin': 'https://chat18.aichatos.xyz/', }
            _json = {"prompt": question, "userId": "#/chat/1699253255434", "network": True, "system": "",
                     "withoutContext": False, "stream": False}
            # 连接超时时间为3s，读取超时时间为3s
            response = requests.post(url='https://chat18.aichatos.xyz/api/generateStream', headers=headers, json=_json,
                                     timeout=(3, 3))
